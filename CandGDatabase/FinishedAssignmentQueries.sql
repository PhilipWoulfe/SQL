...
select Last_Name, First_Name, Employee_No from Employees where Hire_Date <= '1991/05/21' order by Last_Name asc;
...
select Employees.Department_No, Employees.Last_Name, Employees.Annual_Salary from Employees where Hire_Date between '1987/09/16' AND '1996/05/12' order by Department_No, Last_Name asc;
...
Select Employees.Last_Name, cast(round((Employees.Annual_Salary / 12), 2) as decimal(10, 2)) as 'Monthly Salary' from Employees where Employees.Department_No = 50 OR Employees.Department_No = 90 order by Last_Name asc;
...
select CASE WHEN Employees.Department_No=20 OR Employees.Department_No=10																THEN 5
            WHEN Employees.Department_No=50 OR Employees.Department_No=80 OR Employees.Department_No=90 OR Employees.Department_No=110  THEN 10
																																		ELSE 0
			END as 'Percent_Increase(%)', Employees.Employee_No, Employees.Annual_Salary, 
...
CREATE VIEW Managers 
	as SELECT * FROM Employees where Employees.Job_ID like '%MGR%' WITH Check Option;
